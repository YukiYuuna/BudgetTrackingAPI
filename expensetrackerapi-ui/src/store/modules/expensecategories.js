...
const actions = {
    loadExpenseCategories ({ commit }) {
      return ExpenseCategoriesService.getAllExpenseCategories().then(expenseCategories => {
        commit('getAllExpenseCategories', expenseCategories)
      })
    },
...
const mutations = {
    ...
    modifyExpenseCategory (state, expenseCategory) {
        let expenseCategoryUpdated = state.expenseCategories.find(ec => ec.categoryName === expenseCategory.categoryName)
